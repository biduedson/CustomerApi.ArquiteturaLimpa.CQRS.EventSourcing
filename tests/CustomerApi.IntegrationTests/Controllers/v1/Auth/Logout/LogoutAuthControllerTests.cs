...
namespace CustomerApi.IntegrationTests.Controllers.Auth.Logout;

[IntegrationTest]
public class LogoutAuthControllerTests : ControllerTestsBase
{
    ...
    public async Task Should_ReturnsHttpStatus200Ok_When_Post_ValidRequest()
    {
        ...
        AddRefreshTokenCookie(httpClient, "refresh-token");
        ...
        await mediator.Received(1).Send(
            Arg.Is<LogoutCommand>(request => request.RefreshToken == "refresh-token"),
    ...
    public async Task Should_ReturnsHttpStatus400BadRequest_When_Post_InvalidRequest()
    {
        ...
        var mediator = Substitute.For<IMediator>();
        mediator.Send(Arg.Any<LogoutCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result.Error("Refresh token invalido."));
