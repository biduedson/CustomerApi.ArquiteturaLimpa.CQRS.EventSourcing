...
public class CustomersControllerTests : IAsyncLifetime
{
    ...
    private readonly SqliteConnection _writeDbContextSqlite = new(ConnectionString); // Banco para operações de escrita (comandos)

    #region POST: /api/customer/ - Testes para criação de clientes
    ...
    public async Task Should_ReturnsHttpStatus400BadRequest_When_Post_EmailAddressIsAlready()
    {
        var customer = new Faker<Customer>()
            .CustomInstantiator(faker =>
            {
                ...
                return Customer.Create(faker.Person.FirstName,
        ...
        var command = new Faker<CreateCustomerCommand>()
           .RuleFor(command => command.FirstName, faker => faker.Person.FirstName)
           .RuleFor(command => command.LastName, faker => faker.Person.LastName)
           .RuleFor(command => command.Email, customer.Email.Address) // Usa o mesmo email
           .RuleFor(command => command.Gender, faker => faker.PickRandom<EGender>())
           .RuleFor(command => command.DateOfBirth, faker => faker.Person.DateOfBirth)
           .Generate();
    ...
    #region GET: /api/customer/ - Testes para listar todos os clientes
    [Fact]
    public async Task Should_ReturnsHttpStatus200Ok_When_GetAll()
    {
        var queryModels = new Faker<CustomerQueryModel>()
             .UsePrivateConstructor() // Permite usar construtores privados com Bogus
    ...
    #region GET: /api/customer/{id} - Testes para buscar cliente por ID
    [Fact]
    ...
    #region DELETE: /api/customer/{id} - Testes para exclusão de clientes
    ...
    #region IAsyncLifetime - Gerenciamento do ciclo de vida dos testes
    ...
    #region Helpers Métodos auxiliares para configuração dos testes
