...
namespace CustomerApi.IntegrationTests.Controllers.Auth.RefreshToken;

[IntegrationTest]
public class RefreshTokenAuthControllerTests : ControllerTestsBase
{
    private const string Endpoint = "/api/auth/refreshtoken";
    ...
    public async Task Should_ReturnsHttpStatus200Ok_When_Post_ValidRequest()
    {
        ...
        AddRefreshTokenCookie(httpClient, "old-refresh-token");

        // Executa a chamada HTTP.
        using var act = await httpClient.PostAsync(Endpoint, null);
        ...
        await mediator.Received(1).Send(
            Arg.Is<RefreshTokenCommand>(request =>
    ...
    [Fact]
    public async Task Should_ReturnsHttpStatus401Unauthorized_When_Post_Unauthenticated()
    {
        // Prepara o cenário.
    ...
    [Fact]
    public async Task Should_ReturnsHttpStatus400BadRequest_When_Post_InvalidRefreshToken()
    {
        // Prepara o cenário.
        var mediator = Substitute.For<IMediator>();
        mediator.Send(Arg.Any<RefreshTokenCommand>(), Arg.Any<CancellationToken>())
            .Returns(Result<AuthenticationResponse>.Error("Refresh token invalido."));
        ...
        using var httpClient = webApplicationFactory.CreateClient(CreateClientOptions());
        AuthenticateAs(httpClient, UserRole.Operator);
        AddRefreshTokenCookie(httpClient, "invalid-refresh-token");
